...
model_save_dir = base_setting.lstm_model_save_dir + 'with_sample/'
...
class RnnModel(object):
    def __init__(self,
                 num_ntoken, num_ttoken, is_training=True, saved_model=True, kernel='LSTM',
                 batch_size=50,
                 n_embed_dim=1500,
                 t_embed_dim=1500,
    ...
    def build_t_output(self, lstm_output):
        with tf.variable_scope('terminal_softmax'):
            t_weight = tf.Variable(tf.truncated_normal(
                [self.num_ttoken, self.num_hidden_units], stddev=0.1))
            t_bias = tf.Variable(tf.zeros(self.num_ttoken))
        terminal_logits = tf.matmul(lstm_output, tf.transpose(t_weight)) + t_bias
    ...
    def build_tt_loss(self,weights, bias, lstm_output, t_targets):
        # 对于terminal token的预测采用负采样方法加快训练速度
        n_sampled = 1000
        t_loss = tf.nn.sampled_softmax_loss(
            weights, bias, t_targets, lstm_output, num_sampled=n_sampled,
            num_classes=self.num_ttoken, num_true=1)
    ...
    def train(self):
        ...
        self.print_and_log('model info: model with negative sample,'+ \
                           'time_steps:{}, batch_size:{}'.format(self.time_steps, self.batch_size))
